 ...
 *
 * Hosts the domain-tag registry in two parallel forms derived from the same
 * `TAG_BYTES_*` macros:
 *   - Byte form (`domain_tags[][8]`) for runtime absorption.
 *   - U64-packed form for compile-time uniqueness assertions.
 *
 * Two forms exist because a C11 `_Static_assert` needs an integer-constant
 * expression, which a `static const uint8_t[]` is not. `PACK_U64` packs eight
 * char literals into a `uint64_t` ICE that the assertion can compare. The byte
 * form remains the runtime feed; the U64 form exists only for the assertion chain.
 * Single source of truth: each tag has one `TAG_BYTES_*` definition both forms
 * reference.
 */
 ...
 * are never treated as C strings). The "dot-" prefix is a readability marker;
 * the cryptographic effect comes from byte-level distinctness. */
...
/* Pack eight byte values into a uint64_t ICE so a `_Static_assert` can compare
 * tags for inequality. The cast through `unsigned char` preserves the bit pattern
 * when `char` is signed.
 *
 * The PACK_U64 / PACK_U64_IMPL split is the deferred-expansion idiom (C11
 * §6.10.3.1): macro arguments are identified BEFORE expansion, so calling
 * PACK_U64_IMPL directly with `TAG_BYTES_SIV_MAC` (which expands to 8
 * comma-separated bytes) would see one argument, not eight. The variadic outer
 * `PACK_U64(...)` forces the inner expansion during the post-substitution
 * rescan. */
...
/* Pairwise inequality across all CRYPTO_DOMAIN_* tags. With N tags there are
 * N*(N-1)/2 = 10 pairs for N=5; every additional tag adds one new chain of N
 * comparisons. */
...
/* Indexed by `crypto_domain_t`; designated initializers tie row order to the
 * enum even if entries are reordered. */
...
void crypto_mac_init(
     ...
     * monocypher's input size assumptions. */
...
void crypto_mac_absorb(
    ...
    /* Always emit LE64(len) first — even for len == 0 — so ("ab"+"c") and
     * ("a"+"bc") absorb distinct byte streams. */
    uint8_t len_le[8];
    store_le64(len_le, (uint64_t) len);
    crypto_blake2b_update(&ctx->blake, len_le, sizeof(len_le));

    /* `crypto_blake2b_update` accepts (NULL, 0) per monocypher's contract, so
     * len == 0 is safe even when the caller passed NULL. */
    if (len > 0) {
...
void crypto_mac_final(
     ...
     * `crypto_blake2b_final` zeroes its internal accumulator, but the wrapper
     * struct still occupies stack space. See header. */
}
...
void crypto_mac_oneshot(
    ...
    /* NULL = "input not provided", not "absorb LE64(0)" — a single-input call's
     * absorbed bytes must be identical regardless of whether the caller knew
     * about the two-input form. */
    if (input2 != NULL) {
