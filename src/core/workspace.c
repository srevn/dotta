 ...
 *
 * Analysis logic:
 * 1. Check if file exists on filesystem (lstat)
 * 2. Verify file type is supported (regular file or symlink)
 * 3. Check type compatibility (Git vs filesystem)
 * 4. Compare content hashes (Git vs filesystem, transparent decryption)
 * 5. Check Git executable bit (always checked, even without metadata)
 * 6. Check full metadata if available (mode bits + ownership)
 * 7. Determine status: all match → DEPLOYED, any mismatch → PENDING_DEPLOYMENT
 *
 * This function reuses proven infrastructure:
 * - content_hash_from_tree_entry() for Git content hashing
 * - content_hash_file() for filesystem content hashing
 * - check_item_metadata_divergence() for metadata comparison
 *
...
error_t *analyze_file_for_manifest_status(
    ...
    /* Default to pending deployment */
    *out_status = MANIFEST_STATUS_PENDING_DEPLOYMENT;

    /* Step 1: Check if file exists on filesystem */
    struct stat file_stat;
    if (lstat(filesystem_path, &file_stat) != 0) {
        if (errno == ENOENT) {
    ...
    /* Defensive check: Verify we can actually handle this file type */
    if (!S_ISREG(file_stat.st_mode) && !S_ISLNK(file_stat.st_mode)) {
        /* Special file (FIFO, socket, device, etc.) - should never happen in manifest */
        return ERROR(ERR_FS, "unsupported file type: %s", filesystem_path);
    }

    /* Step 2: Check type compatibility (Git vs filesystem) */
    git_filemode_t git_mode = git_tree_entry_filemode(entry);
    bool git_is_symlink = (git_mode == GIT_FILEMODE_LINK);
    bool fs_is_symlink = S_ISLNK(file_stat.st_mode);

    if (git_is_symlink != fs_is_symlink) {
        /* Type mismatch (file ↔ symlink) - needs deployment */
        return NULL;
    }

    /* Step 3: Hash-based content comparison */

    /* Get Git content hash */
    char *git_content_hash = NULL;
    err = content_hash_from_tree_entry(
        repo, entry, storage_path, profile_name,
        metadata, km, NULL, &git_content_hash
    );
    if (err) return err;

    /* Get filesystem content hash */
    char *fs_content_hash = NULL;
    err = content_hash_file(
        filesystem_path, storage_path, profile_name,
        metadata, km, &fs_content_hash
    );
    if (err) {
        free(git_content_hash);
        return err;
    }

    /* Compare hashes */
    bool content_matches = (strcmp(git_content_hash, fs_content_hash) == 0);

    free(git_content_hash);
    free(fs_content_hash);

    if (!content_matches) {
        /* Content differs - needs deployment */
        return NULL;
    }

    /* Step 4: Check metadata divergence */

    /* Get metadata item for this file */
    const metadata_item_t *meta_item = NULL;
    if (metadata) {
        err = metadata_get_item(metadata, storage_path, &meta_item);
    ...
    /* Check Git filemode (executable bit) - ALWAYS checked, even without metadata */
    bool git_is_executable = (git_mode == GIT_FILEMODE_BLOB_EXECUTABLE);
    bool fs_is_executable = (file_stat.st_mode & S_IXUSR) != 0;

    if (git_is_executable != fs_is_executable) {
        has_divergence = true;
    }

    /* Check full metadata if available (all permission bits + ownership) */
    if (!has_divergence && meta_item && meta_item->kind == METADATA_ITEM_FILE) {
    ...
    if (has_divergence) {
        ...
        *out_status = MANIFEST_STATUS_DEPLOYED;
    }
