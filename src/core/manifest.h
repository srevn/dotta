 ...
 * profile's claim already applied (core/row.h). It is computed from Git at
 * every load and never stored — Git × enabled set × mount table → rows is
 * a pure function, and nothing here touches the state database. Surface
 * is two-fold:
 *
