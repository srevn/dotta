 ...
 *
 * Compiles a heterogeneous mix of user inputs (exact paths, directory prefixes,
 * glob patterns) into a single matcher that decides whether a storage path is
 * in the operation's scope.
 *
 ...
 *
 * Matching semantics follow gitignore (via base/gitignore): rule ordering,
 * negation, anchoring, and `**` recursive globs.
 *
 ...
 *   4. Combined glob ruleset evaluation (gitignore last-match-wins semantics;
 *      honours negation across patterns).
 *
 * `kind` is the manifest's kind of the path (a tracked directory squatted by a
 * file is still a directory). Only step 4 reads it: gitignore's directory-only
 * patterns (`dir/`) match a directory itself only when the caller says it is
 * one; files beneath it match via walk-up regardless.
 *
...
/**
 * Free a pathspec. Releases the heap-owned exact-path storage; the arena-borrowed
 * glob storage is reclaimed when the arena is destroyed. Safe with NULL.
 */
...
/**
 * Exact path at index `i`. The pointer borrows into pathspec storage; its lifetime
 * ends at pathspec_free.
 *
 * Iteration order is stable for a given pathspec but otherwise unspecified.
 *
 * `i` MUST be < pathspec_exact_count(spec). Out-of-bounds is undefined behaviour
 ...
 *
 * `i` MUST be < pathspec_glob_count(spec). Out-of-bounds is undefined behaviour
 * (asserted in debug builds).
 */
const char *pathspec_glob_at(const pathspec_t *spec, size_t i);

/**
 * Per-pattern isolated match: tests whether the glob at index `i` matches
 * `storage_path` *as if it were the only rule in the ruleset*.
 *
 * Used by filter-coverage validation: a combined-ruleset evaluation folds one
 * pattern's negation (or shadowing) into another's verdict, which under-counts
 * coverage on overlap. Per-pattern isolation gives each input independent
 * attribution.
 *
 * `kind` as for pathspec_matches. Returns false for a NULL pathspec or NULL path.
 * `i` MUST be < pathspec_glob_count(spec); out-of-bounds is undefined behaviour
 * (asserted in debug builds).
 */
