...
static error_t *apply_update_and_save_state(
    ...
    /*
     * Synchronize directory state with active profiles
     *
     * Directory tracking is deployment-scoped: directories should only be tracked
     * for profiles that are currently active. Remove directory entries that belong
     * to profiles no longer in the active set.
     *
     * This prevents:
     * - Performance degradation (scanning inactive profile directories)
     * - State corruption (orphaned directory entries)
     * - Confusion (untracked files shown for wrong profile context)
     */
    size_t dir_count = 0;
    ...
    if (directories && dir_count > 0) {
        ...
        for (size_t i = 0; i < string_array_size(dirs_to_remove); i++) {
            const char *dir_path = string_array_get(dirs_to_remove, i);

            err = state_remove_directory(state, dir_path);
            if (err) {
                /* Non-fatal: log and continue */
                output_print(out, OUTPUT_VERBOSE,
                            "Warning: Failed to remove orphaned directory '%s': %s\n",
        ...
        if (removed_dir_count > 0) {
            output_print(out, OUTPUT_VERBOSE,
                        "Cleaned up %zu orphaned director%s from state\n",
                        removed_dir_count, removed_dir_count == 1 ? "y" : "ies");
        }
    }
