 ...
 * directory iterations the command runs. The handle caches the last-discovered
 * source repository so repeated queries against the same subtree amortise the
 * `git_repository_discover` walk.
 *
 ...
 *
 * Semantics:
 *   - Returns `*out = false` (no error) when `abs_path` is not inside any git
 *     repository, or the containing repository is bare.
 *   - Returns `*out = true` only when libgit2's `git_ignore_path_is_ignored`
 *     reports a positive match against the containing repo's rules.
 *   - Uses `git_repository_discover` with `across_fs = 0`, so a source repo on
 *     a different filesystem than `abs_path` is treated as "not in a repo" —
 ...
 * input must resolve it first (in-tree callers either ride path_input_normalize,
 * path_input_resolve, realpath, or feed a pre-resolved state filesystem path).
 *
