 ...
 * what dotta did to each managed path. Everything else — what should
 * stand at a path, from whom — is computed from Git at every load
 * (core/manifest.h) and never stored.
 *
 ...
 *   - file_anchors, directory_anchors: per-kind views of anchors, a
 *     debugging window; never read by dotta
 *
 * Design principles:
 ...
 * - Nothing derivable is stored: the record is dotta's own and never
 *   derivable; the expected side is Git's and lives in Git
 */
 ...
 *     disk-matches-blob verification — state_confirm (the slow-path
 *     CMP_EQUAL) and state_anchor (apply deploy, adoption, add, update).
 *     Zero blob_oid is no content confirmation — a directory, whose whole
 *     confirmed-disk record is that it was observed (a directory has no
 *     content confirmation, schema-enforced), or a file observed but
 *     never confirmed.
 *   - deployed_at     : active-ownership timestamp. Advances to now on
 *     every state_anchor (apply deploy, adoption, acknowledgement, add,
 *     update); untouched by a confirmation. 0 = dotta never put this
 *     here.
 ...
 * deployed what, under which claim. A confirmation rewrites only what it
 * confirmed (type, blob, stat); the claim — profile, storage path, mode,
 * owner, group — is an ownership event's to change. They are what an
 * orphan (a record whose path no active row names) is measured against,
 * and an owned record whose profile ≠ the active row's profile is a
 * reassignment apply has not acknowledged.
 *
 ...
 * for an orphan; cleared by state_confirm and state_anchor, one of which
 * every route back under an active row takes once disk matches the row
 * (a clean fast-path hit on an ordered record queues the confirmation
 * for exactly this). While the copy is edited or gone the order stands,
 ...
 * returns an error. Use for the READ acquisition shape — see
 * runtime.h's dotta_state_mode_t.
 *
 ...
 * paths). Use for the WRITE acquisition shape — see runtime.h's
 * dotta_state_mode_t. If another process holds the write lock, waits
 * up to 3 seconds (SQLITE_BUSY).
 *
 * @param repo Repository (must not be NULL)
 * @param out State structure (must not be NULL, caller must free with state_free)
 * @return Error or NULL on success
 ...
 * either acquisition shape (workspace_flush_updates, ...) to decide
 * whether to start their own scoped transaction or piggyback on the
 * caller's.
 *
 ...
 * position column reflects the order of `profiles`. Per-row state (the
 * target) is preserved across the rewrite — only precedence changes.
 *
 ...
 * workspace loads it once per run and indexes it by path; the verbs
 * that need one record by path (remove) load it the same way and index
 * it themselves rather than growing a point read for one caller;
 * manifest_diff reads it to tell a departed row with a record from one
 * without.
 *
 ...
 *
 * The slow path's CMP_EQUAL, persisted: rewrites what the comparison
 * established — the kind (type), the content (blob_oid) and the stat
 * triple captured with it — and nothing of the claim the record carries
 * (profile, storage_path, mode, owner, group), which only an ownership
 * event changes. prune_ordered resets: the path is back under a live
 * row. One UPDATE by filesystem_path; the record must exist — one cannot
 * confirm what one has not seen, and the flush observes first. File rows
 * only: a directory has no content to confirm, and row->blob_oid must be
 * non-zero (a zero blob would record "never confirmed" for a path this
 * call claims to have confirmed — rejected here, where the schema's
 * CHECK would only refuse the zeroblob).
 *
 * Why a confirmation leaves the claim alone: the claim says who deployed
 * the content on disk and under which row. A confirmation against a
 * different profile's row does not change that — the file still holds
 * what profile A put there, it merely also satisfies B's row — and the
 * workspace reads A ≠ B as a reassignment apply has yet to acknowledge
 * (workspace.c analyze_file_divergence). Were the confirmation to
 * rewrite the profile, the slow path would acknowledge reassignments
 * silently while the fast path, which writes nothing, showed them.
 *
 * No snapshot mirror is taken here: the caller that keeps one (the
 * workspace's flush) patches exactly the columns this statement names
 * on the record it already holds.
 *
 * @param state State (must not be NULL, must have open database)
 * @param row Active row whose blob disk was found equal to (must not be
 *            NULL; a file row with a non-zero blob)
 ...
 *
 * The ownership event — apply deploy, adoption, acknowledgement, add,
 * update. Call after confirming disk content matches row->blob_oid (or,
 * for a DIRECTORY row, after creating or confirming the directory). One
 * statement, an UPSERT on anchors: the INSERT arm creates the row with
 * observed_at = now; the UPDATE arm rewrites everything the row and the
 * confirmation supply and leaves observed_at alone. deployed_at = now on
 * both arms.
 *
 ...
 *   - If no workspace is live (add's and update's capture loops), this
 *     function is the legitimate direct caller. There is no snapshot to
 *     patch, so callers pass resolved_out=NULL and the next
 ...
 *     deployed_at = now — plus the one column the SQL decided,
 *     observed_at, read back through RETURNING. Snapshot mirrors assign
 *     it directly; no C-side rule logic is needed because the DB already
 *     applied the rule.
 ...
 *
 * Read in exactly one place — the workspace's orphan analysis — and
 * honoured only for a record whose path is not in the view; cleared by
 * state_confirm and state_anchor, one of which every route back into
 * "active" eventually takes (add, update, deploy, the CMP_EQUAL flush).
 *
 * @param state State (must not be NULL, must have active transaction)
