...
static error_t *prepare_statements(state_t *state) {
    ...
    const char *sql_insert =
        ...
        "(filesystem_path, storage_path, profile, old_profile, git_oid, content_hash, "
        " type, mode, owner, \"group\", encrypted, deployed_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    ...
    const char *sql_update_entry =
        ...
        "storage_path = ?, profile = ?, old_profile = ?, git_oid = ?, content_hash = ?, "
    ...
    const char *sql_get =
        "SELECT storage_path, profile, old_profile, git_oid, content_hash, "
        "type, mode, owner, \"group\", encrypted, deployed_at "
    ...
    const char *sql_by_profile =
        "SELECT filesystem_path, storage_path, profile, old_profile, git_oid, content_hash, "
        "type, mode, owner, \"group\", encrypted, deployed_at "
...
error_t *state_add_file(state_t *state, const state_file_entry_t *entry) {
    ...
    /* Bind parameters - all 12 fields for scope-based schema */
    /* 1. filesystem_path */
...
error_t *state_get_file(
    ...
    /* Extract all 11 columns from scope-based schema */
    ...
    sqlite3_int64 deployed_at = sqlite3_column_int64(stmt, 10);

    /* Validate required columns */
    if (!storage_path || !profile || !git_oid || !content_hash || !type_str) {
        return ERROR(ERR_STATE_INVALID,
...
error_t *state_get_all_files(
    ...
    *count = 0;
    ...
    /* Query all files with scope-based schema (12 columns) */
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW && i < file_count) {
        /* Get all 12 columns with NULL checking (scope-based schema) */
...
error_t *state_get_all_directories(
    ...
    *count = 0;
    ...
    if (!mutable_state->stmt_get_all_directories) {
        const char *sql =
            "SELECT directory_path, storage_prefix, profile, added_at, mode, owner, \"group\", deployed_at "
...
error_t *state_update_directory(
    ...
    if (!state->stmt_update_directory) {
         ...
         *
         * Columns updated: storage_prefix, profile, added_at, mode, owner, group
         * Columns preserved: directory_path (WHERE clause), deployed_at (lifecycle)
         */
        const char *sql =
            "UPDATE tracked_directories "
            "SET storage_prefix = ?, profile = ?, added_at = ?, mode = ?, owner = ?, \"group\" = ? "
    ...
    /* Bind parameters (6 fields + WHERE clause) */
    sqlite3_bind_text(stmt, 1, entry->storage_prefix, -1, SQLITE_TRANSIENT);
    ...
    /* Owner (optional) */
    if (entry->owner) {
        sqlite3_bind_text(stmt, 5, entry->owner, -1, SQLITE_TRANSIENT);
    } else {
        sqlite3_bind_null(stmt, 5);
    }
    ...
    /* Group (optional) */
    if (entry->group) {
        sqlite3_bind_text(stmt, 6, entry->group, -1, SQLITE_TRANSIENT);
    } else {
        sqlite3_bind_null(stmt, 6);
    }

    /* WHERE clause: directory_path */
    sqlite3_bind_text(stmt, 7, entry->directory_path, -1, SQLITE_TRANSIENT);
    ...
    int changes = sqlite3_changes(state->db);
    if (changes == 0) {
        return ERROR(ERR_NOT_FOUND, "Directory not found in state: %s", entry->directory_path);
    }
...
error_t *state_remove_directory(state_t *state, const char *directory_path) {
    CHECK_NULL(state);
    CHECK_NULL(directory_path);
    CHECK_NULL(state->db);

    /* Prepare statement if not already prepared */
    if (!state->stmt_remove_directory) {
        const char *sql = "DELETE FROM tracked_directories WHERE directory_path = ?;";
    ...
    sqlite3_reset(stmt);
    sqlite3_bind_text(stmt, 1, directory_path, -1, SQLITE_TRANSIENT);
 ...
 * @param content_hash Content hash (must not be NULL)
...
error_t *state_update_entry(
    ...
    /* Reset and bind all 11 fields + filesystem_path for WHERE clause */
    ...
    sqlite3_bind_text(state->stmt_update_entry, 12, entry->filesystem_path, -1, SQLITE_TRANSIENT);
