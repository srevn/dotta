 ...
 *
 * One backend on every supported platform. Larger requests loop within the POSIX
 * 2024 256-byte cap; EINTR is retried.
 */
...
error_t *entropy_fill(uint8_t *out, size_t len) {
    ...
    while (off < len) {
        ...
        /* EINTR retry: getentropy is documented as not interruptible by signals
         * on macOS/FreeBSD; on Linux the underlying getrandom(2) can return EINTR
         * while the kernel CSPRNG is still seeding. Either way, retry is the
         * right response. */
        int rc;
        do {
