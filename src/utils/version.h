 ...
 *
 * Thin layer on top of the compile-time constants exposed by `<version.h>` (the
 * project-wide header in `include/`). That header owns the data; this module
 * owns the *presentation* — composing version string, commit, platform, build
 * metadata, and the linked libgit2 version into the banner printed by `dotta
 * --version`.
 *
 * Kept in `utils/` rather than `base/` because the banner's wording and layout
 * are dotta-specific application plumbing (per CLAUDE.md's layer charter), not
 * a generic primitive.
 */
