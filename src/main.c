...
static int cmd_add_main(int argc, char **argv, const config_t *config, output_ctx_t *out) {
    ...
    error_t *err = repo_open(config, &repo, NULL);
    if (err) {
        error_print(err, stderr);
        error_free(err);
        free(files);
...
static int cmd_remove_main(int argc, char **argv, const config_t *config, output_ctx_t *out) {
    ...
    for (int i = 2; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--delete-files") == 0) {
            opts.delete_files = true;
 ...
 * Parse git command
 */
static int cmd_git_main(int argc, char **argv, const config_t *config) {
    /* Open repository and get path */
    git_repository *repo = NULL;
    char *repo_path = NULL;
    error_t *err = repo_open(config, &repo, &repo_path);
    if (err) {
        error_print(err, stderr);
        error_free(err);
        return 1;
...
static int cmd_show_main(int argc, char **argv, const config_t *config, output_ctx_t *out) {
    ...
    /* Open resolved repository */
    git_repository *repo = NULL;
    err = repo_open(config, &repo, NULL);
    if (err) {
        error_print(err, stderr);
        error_free(err);
        ret = 1;
 ...
 */
static int cmd_interactive_main(int argc, char **argv, const config_t *config) {
    /* Parse arguments */
    for (int i = 2; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
 ...
 */
static int cmd_completion_main(int argc, char **argv, const config_t *config) {
...
int main(int argc, char **argv) {
    ...
    if (strcmp(command, "--help") == 0 || strcmp(command, "-h") == 0) {
        ...
        ret = cmd_completion_main(argc, argv, config);
    } else {
