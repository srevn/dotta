 ...
 *   3. seed = MAC(prf_key, CIPHER_KEY, SIV) — keying with `prf_key` keeps the
 *      keystream behind a secret (SIV is public).
 *   4. crypto_chacha20_x(seed, nonce=SIV[0..24], ctr=0) XORs plaintext with
 ...
 * in scope via `<monocypher.h>`); non-crypto layers use `secure_wipe` from
 * `base/secure.h`. Functionally identical doubly-volatile loops.
 */
...
/* Defensive upper bound on storage_path bytes (excluding NUL). Profile-relative
 * paths stay well under 1 KiB; this cap defends the boundary independent of
 ...
 *
 * `strnlen` caps the scan even if the caller hands us a non-NUL-terminated buffer
 * — the crypto module defends its own boundary against caller memory-safety bugs.
 *
 ...
 *
 * Each input is LE64-prefixed by `crypto_mac_absorb`. Used identically by encrypt
 * (plaintext is user input) and decrypt (plaintext is the candidate just XORed
 * from the keystream). Wipes its context before return.
 */
...
error_t *cipher_encrypt(
    ...
    *out_ciphertext = (buffer_t){ 0 };
    ...
    /* Every error exit funnels through `cleanup` so the wipe-and-free discipline
     * is uniform: future code that writes secrets into `keystream_seed` or `output`
     * between validation and the existing `goto cleanup` sites cannot silently
     * leak by introducing a new `return err` above the wipes. The cleanup is
     * idempotent on the zero-initialised stack (keystream_seed is `{0}`, output
    ...
    /* Step 2: SIV over (header, path, plaintext) keyed by mac_key. The plaintext
     * is read here and again at step 4 — SIV is fundamentally two-pass. */
    compute_siv(
        mac_key, header, storage_path, path_len,
        plaintext, plaintext_len, siv_slot
    );

    /* Step 3: keystream seed = MAC(prf_key, CIPHER_KEY, SIV). Keying with prf_key
     * keeps the keystream secret even though the SIV is public (it sits in the
     * output we are about to hand out). */
    crypto_mac_oneshot(
        keystream_seed, prf_key, CRYPTO_DOMAIN_CIPHER_KEY,
        siv_slot, CIPHER_SIV_SIZE, NULL, 0
    );

    /* Step 4: XChaCha20(seed, nonce=siv[0..24], ctr=0) XOR plaintext. monocypher
     * reads exactly 24 bytes from the nonce pointer; the SIV's tail 8 bytes serve
     * only as the MAC tag. plaintext_len == 0 is a no-op inside the primitive. */
    crypto_chacha20_x(
...
cleanup:
    ...
    /* On error, wipe whatever was written into the output buffer before freeing.
     * The bytes might include a freshly-written keystream which, paired with
     * knowledge of the plaintext, would leak the keystream to a memory-disclosure
     * attacker. */
    if (output.data) {
...
error_t *cipher_decrypt(
    ...
    *out_plaintext = (buffer_t){ 0 };
    ...
    /* Same uniform-exit pattern as cipher_encrypt: validation early-returns route
     * through `cleanup` so a future partial-secret write cannot silently bypass
     * the wipe. */

    err = validate_path(storage_path, &path_len);
    if (err) goto cleanup;

    /* Mirror the encrypt-side cap, accounting for OVERHEAD. The compile-time
    ...
    /* Transfer ownership to caller. The cleanup path becomes a no-op because
     * `output` is now zeroed. */
