 ...
 *
 * A profile names what it holds portably: `<label>/<tail>`, the label one of three
 * words and the tail a relative path with no traversal in it. The label says which
 * namespace the name is in — home/ under the invoker's HOME, root/ under `/`,
 * custom/ under the target this machine binds for the profile — and the tail is
 * the name within it, the subject every user-authored pattern is matched against
 ...
 * This module is the words and the shape alone: no table, no arena beyond the one
 * compose, no disk, no identity.
 *
 ...
 * every verb beneath may join a tail onto a root's spelling without looking. The
 * bytes of a name are the cipher's associated data (crypto/cipher.h), so the shape
 * rule refuses a malformed name and folds nothing, and the two recognitions read
 * a prefix or a whole word and validate nothing, their domain being any string
 ...
 *
 * Three verbs pay for that decision. A name is a string of a particular shape and
 * C cannot say so, so label_of, label_tail and label_compose assert what they
 * cannot declare — the two projections where they take a name, the compose where
 * it makes one. The assert is not theatre against an invented bug: this tree holds
 * a bare word in a field typed as a name by design (the health slice's ROOT entry,
 * core/manifest.h), so "looks like a name, is not one" is a real inhabitant here,
 * and one abort per verb is what stands against it where a header sentence is all
 ...
 *
 * A fourth label is an enumerator here, an entry in label_words, one more slot in
 * every array sized by LABEL_COUNT, a decision at every reader that derives a
 * consequence from one — and a hand edit to label_validate_storage's sentence,
 ...
 * zero the answer was built from (gitignore_match_t's shape: one field gates its
 * siblings). "" is a tail: a label spelled as a directory ("home/") stands under
 * its label with nothing past it, and "home//" is a tail of separators. Nothing
 * here folds — a caller that reads a directory spelling as the label alone sheds
 * its own separators first (infra/path.c) — and the tail aliases the input, so
 ...
 *
 * The prefix and its separator, nothing of the tail: the question any string may
 * be asked, NULL included, where the two projections beneath it ask for a path
 * that passed this one. It is the content gate every walk over a profile tree
 * asks of its own walk root — a managed path stands under a label, so a blob at
 * the branch root, or beneath a tree no label names, is the branch's own machinery:
 * dotta's files (.dottaignore, .bootstrap, .dotta/) and whatever else a hand or
 * a tool left beside them. Nothing else distinguishes them, and nothing needs to:
 * a branch may hold what it likes next to the labels, and no walk of content sees
 * it. A caller *handed* such a name rather than finding it refuses in its own
 * words — export's name arm, which takes a label as a key and meets whatever stands
 * there (cmds/export.c). And it is the shape dispatch on an argument, which reads
 * a storage shape before the filesystem shapes. A label alone, with no separator,
 * stands under none: that is the whole-word question, label_parse, and a caller
 * wanting either asks both (cmds/export.c's grammar). A label spelled as a directory
 * does stand under one — "custom/" is the label and an empty tail — so a gate meant
 * to catch a label asks it of the word.
 *
 ...
 * diff's delta selection (cmds/diff.c), the rule compiler (infra/pathspec.c), the
 * resolver's storage arm and the question its neighbour asks of a positional whose
 * slot is undecided (infra/path.c path_input_resolve, path_input_announces_path),
 * the two input heads that dispatch on shape before reading it (cmds/add.c,
 * cmds/ignore.c) and export's two — its profile slot's own grammar, and the gate
 * its name arm asks of the key it is handed (cmds/export.c).
 */
 ...
 * pattern's, and nothing above the mount root takes part. Paths are resolved through
 * the table (infra/mount.h); patterns are not.
 *
 * Zero allocation; the returned pointer aliases `storage_path` and shares its
 * lifetime. label_of's precondition, asserted the same way: every reader holds a
 * composed or a validated path, and a tail of something that stands under no label
 * is not an answer.
 *
 ...
 * the union at its own site (cmds/export.c's grammar, where a lone positional that
 * is content rather than a profile is one or the other).
 *
 ...
 *
 * The order is the sentence's, not the rule's: an empty path and an absolute one
 * are refused in their own words above the label test, which would otherwise
 ...
 * manifest_name). Asserted, as the two projections assert the same rule read from
 * the other side.
 *
 * Reader: the last rung of the namer's ascent, which is the one place in the tree
 * a location becomes a name (core/manifest.c, through infra/mount.h mount_name).
 *
