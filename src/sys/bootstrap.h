 ...
 *
 * Pure content operations on per-profile .bootstrap scripts stored in Git trees.
 * Every function here is a single-responsibility primitive — Git blob read,
 * temp-file extraction, or shebang validation — with no knowledge of iteration,
 * progress output, subprocess spawning, or configuration.
 *
 * Orchestration (filtering, per-script progress, dry-run, aggregated failure
 * reporting) lives in utils/bootstrap.h on top of these primitives and the unified
 * subprocess primitive in sys/process.h.
 */
 ...
 *
 * Always lives at the root of a profile's Git tree. Dotta does not support
 * alternate names; the constant exists to avoid scattering the literal ".bootstrap"
 * through progress messages and UI code.
 */
 ...
 *
 * Swallows errors — a missing profile, an unreadable tree, or a non-existent
 * entry all collapse to `false`. Callers use this to drive UI decisions (show
 * ✓/✗, filter which profiles to execute) where "no script" is not a fatal
 * condition.
 *
...
/**
 * Extract a profile's .bootstrap script to a secure temporary file with mode 0700.
 *
 * On success, *out_temp_path is a heap-allocated path the caller must unlink()
 * and free() when finished. On failure, no temp file is left behind — the
 * implementation cleans up any partial state.
 *
 ...
 *
 * Content is inspected bytewise; size is authoritative (the buffer is not required
