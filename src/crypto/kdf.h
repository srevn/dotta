 ...
 *                                            (two domain-separated keyed-BLAKE2b
 *                                             calls)
 *
 ...
 * (validation, framing, mlock attempts) is hygiene around that one primitive call.
 *
 ...
 * of ~8 KiB). `_MAX` is a DoS bound (see file-level "Argon2 parameter bounds").
 * All values fit `uint16_t` / `uint8_t` for the on-disk blob header.
 */
 ...
 *
 * Argon2id (RFC 9106), single-lane. The 32-byte `salt` is the per-repository
 * random tag stored at `refs/dotta/salt:salt`; uniqueness across repositories
 * forecloses cross-installation precomputation attacks (a precomputed table built
 * against one dotta repo is useless against any other).
 *
 ...
 *
 * Validates `memory_mib` / `passes` against KDF_ARGON2_*_MIN/MAX before allocation,
 * and `passphrase_len` against UINT32_MAX (Argon2's representational limit) to
 * prevent silent truncation. Every error path scrubs `out_master_key` before
 * return.
 *
 ...
 * distinct domain tags. Returns `error_t *` rather than `void` for forward-compat
 * (today `crypto_mac_oneshot` cannot fail). Output buffers are wiped on any error
 * path.
 *
