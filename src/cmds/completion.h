 ...
 *
 * Two commands with opposite contracts, and the library of sources every command's
 * `complete` hook draws on:
 *
 *   dotta completion <shell> Prints the completion script for <shell> — fish is
 *       the one supported — generated from this binary's command registry: the
 *       flag and subcommand rules, the condition helpers and the wrapper that
 *       asks `dotta __complete`. Build-time, no repository; a registry name the
 *       script cannot carry as a shell word is an error, reported. `make
 *       completions` writes it to build/completions/dotta.fish.
 *
 *   dotta __complete [--current=<token>] -- <tokens>... Hidden. What can stand
 *       at the cursor of a command line: <tokens> are the complete ones after
 *       `dotta`, <token> the one being typed. The line is consumed as the parser
 *       would consume it and the command's hook answers
 *       (`args_complete_candidates`). The script's wrapper is the caller, and
 *       it reads silence as "no candidates": never an error message, nothing on
 *       failure, exit 0 outside a repository — where every source prints nothing,
 *       and only native path requests (ARGS_WANT_FILES / ARGS_WANT_DIRS), which
 *       need none, come back.
 *
 ...
 * a profile reads or writes; ALL adds what `profile fetch` would download. Branch
 * order is the ref iteration's; the shell sorts.
 */
 ...
 * win is not offered because the verb would skip it. Directory rows ride along
 * slash-marked when asked for — a directory claim is a subtree filter with prefix
 * semantics for the verbs that act on it (apply, update, diff); the file-only
 * form serves the slots with nothing to show for one (list's history).
 *
...
/**
 * The profile a token names: its part before ':' when it is a refspec, the token
 * itself otherwise (its commit part, `profile@commit`, dropped) — what a first
 * positional pins for the verbs that read `[profile:]path` shapes. The refspec
 * parser draws the line, as the commands do. Arena-owned or the token itself;
 * never NULL.
 */
const char *completion_profile_of(const dotta_ctx_t *ctx, const char *token);

/**
 * Commits: the reference forms, valid against any branch, then the recent history
 * of the branches named — one that names no branch contributes nothing — or,
 * when none is named or none is a branch, of the enabled set in precedence order,
 * where show and diff resolve a bare reference. Interleaved histories are labelled
 * by branch.
 */
...
/**
 * The history of one pinned profile — the same candidates as `completion_commits`
 * for that branch alone; NULL, or a name that is no branch (a path handed in as
 * a guess), falls through to the enabled histories.
 */
 ...
 *
 * Hidden from top-level help and from the completion export; silent on failure.
 * Registered in main.c's static `dotta_commands[]`; defined in completion.c beside
